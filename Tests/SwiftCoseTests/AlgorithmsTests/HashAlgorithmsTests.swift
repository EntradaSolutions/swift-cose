...
final class HashAlgorithmsTests: XCTestCase {

    func testSha1() {
        let data = "test".data(using: .utf8)!
        let hashAlgorithm = Sha1()
        
        let hash = hashAlgorithm.computeHash(data: data)
        let expectedHash = "a94a8fe5ccb19ba61c4c0873d391e987982fbbd3".hexStringToData!
        
        XCTAssertEqual(hash, expectedHash, "SHA-1 hash does not match expected value.")
    }
    ...
    func testSha256() {
        ...
        XCTAssertEqual(hash, expectedHash, "SHA-256 hash does not match expected value.")
    }

    func testSha256Trunc64() {
        let data = "test".data(using: .utf8)!
        ...
        XCTAssertEqual(hash, expectedHash, "SHA-256/64 truncated hash does not match expected value.")
    }
    ...
    func testSha384() {
        ...
        XCTAssertEqual(hash, expectedHash, "SHA-384 hash does not match expected value.")
    }
    ...
    func testSha512() {
        ...
        XCTAssertEqual(hash, expectedHash, "SHA-512 hash does not match expected value.")
    }
    ...
    func testShake128() {
        ...
        print("Actual Hash: \(hash.toHex)")
        print("Expected Hash: \(expectedHash.toHex)")
        
        XCTAssertEqual(hash.count, 256 / 8, "SHAKE-128 hash length does not match expected value.")
        XCTAssertEqual(hash, expectedHash, "SHAKE-128 hash does not match expected value.")
    }
    ...
    func testShake256() {
        ...
        XCTAssertEqual(hash.count, 512 / 8, "SHAKE-256 hash length does not match expected value.")
        XCTAssertEqual(hash, expectedHash, "SHAKE-256 hash does not match expected value.")
    }
}
